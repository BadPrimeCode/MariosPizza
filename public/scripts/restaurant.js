...
$(document).ready(function() {
  ...
  $('#createTable').on('click',function() {
    ...
    if (lastAssigned>(firstId+(employees.length-1))) {
      ...
      success: function( data ){
        ...
        for (var i = 0; i < tables.length; i++) {
          ...
          for (var j = 0; j < employees.length; j++) {
            if (tables[i].first_name === employees[j].first_name){
              selectText += '<option selected="selected" value=' + employees[j].id + '>' + employees[j].first_name + ' ' + employees[j].last_name + '</option>';
            } else {
              selectText += '<option value=' + i + '>' + employees[j].first_name + ' ' + employees[j].last_name + '</option>';
            }
          }
          selectText += '</select>';
